...
class TestProxy(object):
	...
	def checkProxyFormat(self,proxy):
		...
		flag = ip.split('.')[0] in list(map(str,range(1,256))) and flag
		flag = ip.split('.')[1] in list(map(str,range(256))) and flag
		flag = ip.split('.')[2] in list(map(str,range(256))) and flag
		flag = ip.split('.')[3] in list(map(str,range(1,255))) and flag
		flag = protocol in ['http','https'] and flag
		flag = port in list(map(str,list(range(1,65535)))) and flag
		'''这里是在检查proxy的格式 '''
		if flag:
